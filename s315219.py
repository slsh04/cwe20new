...
class BaseS3StorageDriver(StorageDriver):
    ...
    def list_container_objects(self, container, ex_prefix=None):
        ...
        """
        return list(self.iterate_container_objects(container, ex_prefix=ex_prefix))
    ...
    def _upload_multipart(
        ...
        try:
            ...
            result = self._upload_from_iterator(
                iterator, object_path, upload_id, calculate_hash
            )
    ...
    def _upload_from_iterator(
        self, iterator, object_path, upload_id, calculate_hash=True
    ):
        """
        ...
        for data in read_in_chunks(
            ...
            resp = self.connection.request(
                request_path, method="PUT", data=data, headers=headers
            )
    ...
    def _commit_multipart(self, object_path, upload_id, chunks):
        ...
        params = {"uploadId": upload_id}
        request_path = "?".join((object_path, urlencode(params)))
        response = self.connection.request(request_path, data=data, method="POST")
    ...
    def upload_object_via_stream(
        self, iterator, container, object_name, extra=None, ex_storage_class=None
    ):
        """
...
class S3APNE2Connection(SignedAWSConnection, BaseS3Connection):
    ...
    def __init__(
        ...
        super(S3APNE2Connection, self).__init__(
            user_id,
            key,
            secure,
            host,
